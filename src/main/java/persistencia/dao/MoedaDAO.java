...
public class MoedaDAO {
    ...
    public List<HistoricoMoeda> selectHistoricoMoeda(int idMoeda, Date dataFinal, Date dataInicial) throws BDException {
        ...
        try (Connection conexao = MySQLConexao.conectar()) {
            try (PreparedStatement preparedStatement = conexao.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                preparedStatement.setInt(1, idMoeda);
                preparedStatement.setDate(2, dataInicial);
                preparedStatement.setDate(3, dataFinal);
                ResultSet resultSet = preparedStatement.executeQuery();
                while (resultSet.next()) {
